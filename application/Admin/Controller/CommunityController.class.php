		...
		if(IS_POST){
            ...
            */
			...
			if($url!="" || $imgs !=''){
				...
				if($imgs){
					if(strpos($imgs,'http')!==false||strpos($imgs,'https')!==false){

						$community_type2=substr(strrchr($imgs, '.'), 1);
  ...
  function cateadd_post(){
	...
	function edit(){
		...
		if($id){
			...
			if($community['imgs']){
				$community['imgs']=explode(',', $community['imgs']);
			}
			if($community['videolink']){
				$community['thumb'] = explode(',',$community['videolink'])[0];
				$community['videolink'] = explode(',',$community['videolink'])[1];
			}
	...
	function edit_post(){
		...
		// }
			...
			if($result!==false){
				...
				$this->success('修改成功','Admin/community/index',3);
			 }else{
				$this->error('修改失败');
			 }
		}			
...
//
    ...
    public function  community_listen(){
    	...
    	if(!$id||$id==""||!is_numeric($id)){
    		...
    		$info['videolink'] = explode(',',$info['videolink'])[1];
