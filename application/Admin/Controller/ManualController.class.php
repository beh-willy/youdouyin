...
class ManualController extends AdminbaseController {
    ...
    public function add_post() { 
        ...
        $coin = $_POST['coin'];
                ...
                $map['id'] = $user_login;
                $map['_logic'] = 'OR';
				$uid=$users_obj->where($map)->getField("id");
				if(!$uid){
        ...
        $result1 = M("users_charge_admin")->add(array("touid"=>$uid,"coin"=>$coin,"addtime"=>time(),"admin"=>$user['user_login'],"ip"=>$_SERVER['REMOTE_ADDR']));
		$result2 = M("users")->where("id='$uid'")->setInc("coin",$coin);
        if ($result1 && $result2) {
			M()->commit();		  
