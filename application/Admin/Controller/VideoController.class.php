...
class VideoController extends AdminbaseController {
    ...
    function cateindex(){
        ...
        $map['isdel']=0;
        $map['status']=1;
        $map['cate'] = array('gt',0);//视频列表包含频道编号
        if($_REQUEST['keyword']!=''){
            $map['uid|id']=array("eq",$_REQUEST['keyword']);
            $_GET['keyword']=$_REQUEST['keyword'];
        }
    ...
    function del(){
        ...
        if($result!==false){
            ...
            if($videoInfo['isdel']==0){ //视频上架情况下被删除发送通知
                ...
                $baseMsg='您于'.date("Y-m-d H:i:s",$videoInfo['addtime']).'上传的《'.$videoInfo['title'].'》视频被管理员于'.date("Y-m-d H:i:s",time()).'删除';
    ...
    function add_post(){
        if(IS_POST){
            ...
            //$video->thumb=$_POST['thumb'] ?$_POST['thumb']:'';
            $video->cate =$_POST["cate"];
            if($url!=""){
            ...
            // if (1111) {//如果视频处理失败则删除云存储
            // 	$info = $upload->del($url);
            // }

            /**
             *  后台视频必须提交所属直播分类，记录一、二级分类id
             */
            if($_POST["cate"]=='')
            {
            ...
            $video->thumb = $this->get_video_cover($url)?$this->get_video_cover($url):$_SERVER['SERVER_NAME'].'/public/images/default_cover.png';
            $video->thumb_s =$video->thumb.'?imageView2/2/w/200/h/200';

            $result=$video->add();

            if($result){
                $this->success('添加成功','Admin/Video/add',3);
            }else{
                $this->error('添加失败');
            }
        }
