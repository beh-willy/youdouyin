...
class IndexadminController extends AdminbaseController {
    ...
    function index(){
    	...
    	$this->assign('nameCoin', $configPub['name_coin']);
     ...
     function del(){
    	...
    	if ($id) {
    		...
    		if ($rst!==false) {
                    ...
                    M("users_sharereg")->where("uid='{$id}' or touid='{$id}'")->delete();
	...
	function sharereg(){
		...
		$map['user_type']=2;
		 ...
		 if($_REQUEST['keyword']!=''){
			$where['uid|touid']=array("like","%".$_REQUEST['keyword']."%");
			$where['_logic']	="or";
			$map['_complex']=$where;
    	...
    	//var_dump($users_model->getlastSql());

    	foreach ($lists as $k => $v) {
    		$lists[$k]['userinfo']=M("users")->where("id={$v['uid']}")->field("user_nicename,id")->find();
    		$lists[$k]['touserinfo']=M("users")->where("id={$v['touid']}")->field("user_nicename,id")->find();
    	}
