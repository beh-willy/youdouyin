    ...
    public function swfupload() {
        if (IS_POST) {
            ...
            $config=array(
            		...
            		'exts'       =>    array('jpg', 'gif', 'png', 'jpeg',"txt",'zip','mp4'),
            		'autoSub'    =>    false,
            );
		...
		/* 		$ret = $cosApi->createFolder($bucket, $folder);
				...
				//$url = $ret['data']['source_url']; //源地址
				$url = $ret['data']['access_url']; //加速地址
				...
				$info=$upload->upload();
				if($info){
					$url = 'http://'.$_SERVER['HTTP_HOST'].'/data/upload/'.$savepath.$info["Filedata"]["savename"]; //加速地址
					echo "1," . $url.",".'1,'.$info["Filedata"]["savename"];
				}else{
