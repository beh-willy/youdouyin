...
require_once '../../api/PhalApi/NotORM/NotORM.php';
$NotORM = new NotORM();
		$NotORM->getORM()->transaction('BEGIN');
			/*$this->getORM()->transaction('BEGIN');*/

				$res1 = DI()->notorm->users_report
					->insert(array("uid"=>111,"touid"=>222,'content'=>'hiwsohnosldnfg','addtime'=>time() ) );
					$result=DI()->notorm->users_liverecord->insert($info);	

				$res2 = DI()->notorm->users
					->where('id = 1')
					->update( array('coin' => new NotORM_Literal("votes + 999")));
		if($res1 && $res2){
			$NotORM->getORM()->transaction('COMMIT');
		}else{
			$NotORM->getORM()->transaction('ROLLBACK');
		}
